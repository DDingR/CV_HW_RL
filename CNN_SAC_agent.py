...
from tensorflow.keras.layers import Dense, Conv2D, Lambda, Flatten, Input, concatenate
...
class Actor(tf.keras.Model):
    def __init__(self, state_dim, action_dim, action_bound):
        ...
        self.conv1 = Conv2D(32, (4,4), strides=(2,2), activation="relu", input_shape=self.state_dim)
        self.conv2 = Conv2D(64, (4,4), strides=(2,2), activation="relu")
        # self.conv3 = Conv2D(64, (3,3), strides=(4,4), activation="relu") 
...
class Critic(tf.keras.Model):
    def __init__(self, state_dim):
        ...
        self.action_h2 = Dense(64, activation='relu')
        self.fc2 = Dense(128, activation='relu')
        self.fc3 = Dense(128, activation='relu')
    ...
    def call(self, x):
        ...
        # h1 = self.conv3(h1)
        h1 = self.flatten(h1)
        h2 = self.action_h2(x[1])
        x = concatenate([h1, h2], axis=-1)
        x = self.fc2(x)
        x = self.fc3(x)
        y = self.fc_out(x)
        return y
...
class CNN_SAC_Agent:
    def __init__(self, state_dim, action_dim, action_bound, args):
        ...
        self.Critic_model_1 = Critic(self.state_dim) 
        self.Critic_model_2 = Critic(self.state_dim)
        self.Critic_model_1_target = Critic(self.state_dim)
        self.Critic_model_2_target = Critic(self.state_dim)
